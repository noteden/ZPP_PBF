<x-layouts::app :title="__('Dashboard')">
    @php
        ...
        $stats = [
            ['label' => 'Gracze',   'value' => \App\Models\User::where('approved', true)->count(), 'icon' => 'users',                  'route' => route('leaderboard.index')],
            ['label' => 'Postacie', 'value' => \App\Models\Charakter::count(),                       'icon' => 'user-group',             'route' => route('character.index')],
            ['label' => 'Posty',    'value' => \App\Models\Post::count(),                            'icon' => 'chat-bubble-left-right', 'route' => route('forum.index')],
            ['label' => 'Misje',    'value' => \App\Models\Mission::count(),                         'icon' => 'map',                    'route' => route('missions.index')],
        ];

        $recentPosts = \App\Models\Post::with(['user', 'thread', 'charakter'])->latest()->limit(5)->get();

        $trending = \App\Models\Thread::withCount('posts')
            ->where('archived', false)->orderByDesc('posts_count')->limit(4)->get();

        $upcomingEvents = \App\Models\Event::where('date', '>=', now())->orderBy('date')->limit(4)->get();

        $myCharacters = $user->charakters()->orderByDesc('experience')->limit(3)->get();
    @endphp
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2 mt-1">
                            ...
                            <flux:text class="text-sm text-zinc-500">Miłej gry w świecie PBF.</flux:text>
                        </div>
                    </div>
                </div>
